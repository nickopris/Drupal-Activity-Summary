...
- A content type `ai_module` with a plain text field `field_module_machine_name` — the module's drupal.org machine name (e.g. `ai`, `ai_agents`). One node per project. Only published nodes are iterated.
- Module enabled: `ddev drush en issue_analysis -y`
- Your drupal Gitlab token - get it at [https://git.drupalcode.org/-/user_settings/personal_access_tokens](https://git.drupalcode.org/-/user_settings/personal_access_tokens) with `read_api` scope.
- Add the following secrets to the active `settings.php` file:
  ```php
  $settings['gitlab_token'] = 'your-token-here';         // https://git.drupalcode.org/ read_api token
